...
- 主题应简短说明唯一工作目标；
- 一个独立任务使用一个分支，不为每条消息或每次对话重复创建分支。
...
Maker 明确确认本任务 IDE Diff 后，该确认同时授权 Agent 按第 10.6 节完成本任务的提交、推送、合并和已合并分支清理，不需要对闭环中的每条常规 Git 命令重复请求确认。

该授权始终不包含 force push、重写 `main` 历史、删除未合并分支、丢弃改动、绕过分支保护或自行解决具有方向性取舍的冲突。
...
标准新任务入口是：干净且与远端同步的 `main`。不满足时，Agent 先提醒 Maker 处理上一任务，不启动无关写入工作。
...
只有 Maker 明确表示接受当前 Diff、可以提交或可以合并，任务才能进入 Accepted。

### 10.8 验收后的自动 Git 闭环

Maker 确认 Diff 后，Agent 自动：
...
4. 推送工作分支到 `origin`；
5. 获取远端最新状态；
6. 确认 `origin/main` 是当前工作分支基线的祖先，没有未知主分支变更；
7. 切换到 `main` 并使用 fast-forward-only 方式同步 `origin/main`；
8. 将工作分支 squash 合并到 `main`；
9. 创建一个代表本任务的主分支提交；
10. 推送 `main` 到 `origin`；
11. 验证本地 `main` 与 `origin/main` 一致且工作区干净；
12. 删除已成功合并的本地工作分支；
13. 删除已成功合并的远端工作分支；
14. 向 Maker 报告最终提交、验证结果、推送和清理结果。

任一步骤遇到验证失败、未知远端提交、冲突、分支保护、权限问题或需要破坏性 Git 操作时，Agent 停止自动闭环并报告，不使用 force push，不重写历史，不删除未合并分支。
